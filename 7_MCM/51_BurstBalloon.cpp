...
int maxCoins_recursion(vector<int> &a) {
    int n = a.size();
    a.push_back(1);
    a.insert(a.begin(), 1);
    return f(1, n, a);
...
int memoization(int i, int j, vector<int> &a, vector<vector<int>> &dp) {
    ...
    for (int ind = i; ind <= j; ind++) {
        int cost = a[i - 1] * a[ind] * a[j + 1] +
                   memoization(i, ind - 1, a, dp) + memoization(ind + 1, j, a, dp);
        maxi = max(maxi, cost);
    }
    return dp[i][j] = maxi;
}
int maxCoins_memo(vector<int> &a) {
    int n = a.size();
    a.push_back(1);
    a.insert(a.begin(), 1);
...
/****************************************************************************/
int maxCoins_tabulation(vector<int> &a) {
    int n = a.size();
