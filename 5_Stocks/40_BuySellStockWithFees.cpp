...
int solve(int i, int buy, int fees, vector<int>& prices, vector<vector<int>>& dp){
    // base case
    if(i >= prices.size()){
        return 0;
    }
    ...
    if(buy == 0){
        // no transaction
        option1 = solve(i + 1, buy, fees, prices, dp);
        // buy the stock on ith day
        option2 = -prices[i] + solve(i + 1, !buy, fees, prices, dp);
    }   
    // we can sell stock
    if(buy == 1){
        // no transaction
        ...
        option2 = (prices[i] - fees) + solve(i + 1, !buy, fees, prices, dp);
    }
...
int maxProfit(vector<int>& prices, int fees){
    ...
    return solve(0, 0, fees, prices, dp);
}
    ...
    for (int i = n - 1; i >= 0; i--){
        for (int buy = 0; buy <= 1; buy++){
            ...
            if(buy == 1){
                ...
                option1 = dp[i + 1][buy];
                // sell the stock on ith day
                option2 = prices[i] - fees + dp[i + 1][!buy];
            }
